...
def tensor_to_file(ts, file_name="case.txt", cal=False):
    if ts.device.type == 'cuda':
        # 在 GPU 上
        num_vec = ts.cpu().numpy()
    else:
        num_vec = ts.numpy()  # tensor to numpy
    
    if cal:
        res = num_vec[:, 0] - num_vec[:, 1]
        sel_rows_idx = np.where(res < 0.3)[0]
        print("select rows (col[0] - col[1] < 0.3):")
        print(sel_rows_idx)
        print("\n")
        sel_rows_idx = np.where(res < 0.2)[0]
        print("select rows (col[0] - col[1] < 0.2):")
        print(sel_rows_idx)
...
def sort_and_rank(score, target):
    ...
    sorted_score, indices = torch.sort(score, dim=1, descending=True) # indices: 实体编号按照score从大到小排列
    a = sorted_score[:, :10]
    b = indices[:, :10]
...
def get_total_rank(test_triples, score, all_ans, eval_bz, rel_predict, cased=False):
    '''
    ...
    if test_triples.shape[1] > 3:
        test_triples = test_triples[:, :3]
    ...
    for idx in range(n_batch): # 对于一个时间戳内的每一个batch
        ...
        ranked, a, b = sort_and_rank(score_batch, target)
        rank.append(ranked) # (batch_size) 一个batch中每一个事实三元组的ground truth排名
        ...
        filter_ranked, filter_a, filter_b = sort_and_rank(filter_score_batch, target)
        filter_rank.append(filter_ranked)
    ...
    if cased:
        tensor_to_file(a, "case_raw_a_2.txt")
        tensor_to_file(b, "case_raw_b_2.txt")
        tensor_to_file(filter_a, "case_filter_a_2.txt", cal=True)
        tensor_to_file(filter_b, "case_filter_b_2.txt")
...
def load_all_answers_for_time_filter(total_data, num_rels, num_nodes, rel_p=False):
    all_ans_list = []
    all_snap, no_use = split_by_time(total_data) # 按时间戳划分的array：[[[s, r, o], [], ...], [], ...]
