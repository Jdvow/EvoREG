...
class TimeConvTransE(torch.nn.Module):
    def __init__(self, num_entities, embedding_dim, input_dropout=0, hidden_dropout=0, feature_map_dropout=0, channels=50, kernel_size=3, sequence_len = 10, use_bias=True):
        ...
        self.bn2_list = torch.nn.ModuleList()
        for _ in range(sequence_len):
            self.conv_list.append(torch.nn.Conv1d(4, channels, kernel_size, stride=1, padding=int(math.floor(kernel_size / 2))))  # kernel size is odd, then padding = math.floor(kernel_size/2))
            self.bn0_list.append(torch.nn.BatchNorm1d(4))
            self.bn1_list.append(torch.nn.BatchNorm1d(channels))
            self.bn2_list.append(torch.nn.BatchNorm1d(embedding_dim))
        self.fc = torch.nn.Linear(embedding_dim * channels, embedding_dim)

    def forward(self, embedding, emb_rels, emb_times, triplets, nodes_id=None, mode="train", negative_rate=0, partial_embeding=None):
        '''
        传入每个历史子图中的实体和关系的分布式embedding，并读取valid或者test集中记录每个事实的历史重复事实的本地文件
        :param embedding: (num_ents, h_dim) 在gpu上
        :param emb_rels: (num_rels*2, h_dim)
        ...
        for idx in range(len(embedding)):
            ...
            time_1, time_2 = emb_times
            time_1 = time_1.unsqueeze(1)
            time_2 = time_2.unsqueeze(1)

            stacked_inputs = torch.cat([e1_embedded, rel_embedded, time_1, time_2], 1)
            stacked_inputs = self.bn0_list[idx](stacked_inputs)
            x = self.inp_drop(stacked_inputs)
            x = self.conv_list[idx](x)
            x = self.bn1_list[idx](x)
...
class TimeConvTransR(torch.nn.Module):
    ...
    def forward(self, embedding, emb_rels, emb_times, triplets, nodes_id=None, mode="train", negative_rate=0):
        '''
        :param embedding: (num_ents, h_dim)
        :param emb_rel: (num_rels*2, h_dim)
        :param triplets: 测试集一个时间戳内的所有事实（包含逆关系）
        ...
        for idx in range(len(embedding)):
            ...
            stacked_inputs = torch.cat([e1_embedded, e2_embedded, time_1, time_2], 1)
            stacked_inputs = self.bn0_list[idx](stacked_inputs)
            x = self.inp_drop(stacked_inputs)
            x = self.conv_list[idx](x)
            x = self.bn1_list[idx](x)
