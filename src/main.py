...
def graph_to_device(glist, super_glist, use_cude, gpu):
    if use_cude:
        if isinstance(glist, list) and isinstance(super_glist, list):
            for g, r_g in zip(glist, super_glist):
                # r_to_e = g.r_to_e
                g = g.to(gpu)
                g.r_to_e = torch.from_numpy(np.array(g.r_to_e))
                # r_to_e = r_g.r_to_e
                r_g = r_g.to(gpu)
                r_g.r_to_e = torch.from_numpy(np.array(r_g.r_to_e))
        elif isinstance(glist, dgl.DGLGraph) and isinstance(super_glist, dgl.DGLGraph):
            # r_to_e = glist.r_to_e
            glist = glist.to(gpu)
            glist.r_to_e = torch.from_numpy(np.array(glist.r_to_e))
            # r_to_e = super_glist.r_to_e
            super_glist = super_glist.to(gpu)
            super_glist.r_to_e = torch.from_numpy(np.array(super_glist.r_to_e))
        else:
            raise TypeError("the parameter is neither a list or a dgl.DGLGraph object")
...
def continual_test(model, history_list, data_list, num_rels, num_nodes, glist, super_glist, use_cuda, all_ans_list, all_ans_r_list, model_name, mode, static_graph):
    """
    用于online setting
    ...
    start_idx = len(history_list)  # 178
    ...
    for time_idx, test_snap in enumerate(data_list):
        ...
        #     pass
        # else:
        temp_his_list_left, temp_his_list_right = tc -2 - args.test_history_len, tc
        if time_idx - 2 - args.test_history_len < 0:
            history_list_temp = [snap for snap in history_list[time_idx - 2 - args.test_history_len:]] + [snap for snap in data_list[0: time_idx]]
        else:
        ...
        ft_history_glist = glist[temp_his_list_left + 1 : temp_his_list_right - 1]
        ft_history_super_glist = super_glist[temp_his_list_left + 1 : temp_his_list_right - 1]
        graph_to_device(ft_history_glist, ft_history_super_glist, use_cuda, args.gpu)
        ...
        valid_history_glist = glist[temp_his_list_left : temp_his_list_right - 2]
        valid_history_super_glist = super_glist[temp_his_list_left : temp_his_list_right - 2]
        graph_to_device(valid_history_glist, valid_history_super_glist, use_cuda, args.gpu)
        ...
        test_history_glist = glist[temp_his_list_left + 2 : temp_his_list_right]
        test_history_super_glist = super_glist[temp_his_list_left + 2 : temp_his_list_right]
        graph_to_device(test_history_glist, test_history_super_glist, use_cuda, args.gpu)
...
def test(model, history_list, test_list, num_rels, num_nodes, glist, super_glist, use_cuda,
         all_ans_list, all_ans_r_list, model_name, mode, static_graph):
    '''
    :param model:
    ...
    his_snap_lens = len(history_list)
    ...
    for time_idx, test_snap in enumerate(tqdm(test_list)): # 对于每一个待测试的时间戳 snapshot_list，按时间戳划分的事实array：[[[s, r, o], [], ...], [], ...]
        ...
        left_idx, right_idx = his_snap_lens + time_idx - args.test_history_len, his_snap_lens + time_idx

        history_glist = glist[left_idx : right_idx]
        history_super_glist = super_glist[left_idx : right_idx]
        graph_to_device(history_glist, history_super_glist, use_cuda, args.gpu)
...
def run_experiment(args, n_hidden=None, n_layers=None, dropout=None, n_bases=None):
    ...
    dir_name = suffix
    # set logger
    log_name = "a-ly{}-his{}-dp{}|{}|{}|{}-gpu{}.txt"\
        .format(args.n_layers, args.train_history_len, args.dropout, 
                args.input_dropout, args.hidden_dropout, args.feat_dropout, args.gpu)
    log_file_path = "../models/{}/{}/{}".format(args.dataset, dir_name, log_name)
    utils.mkdir_if_not_exist(log_file_path)
    sys.stdout = utils.Logger(log_file_path)
    ...
    train_list, train_times = utils.split_by_time(data.train) # 列表，按时间戳划分的事实array：[[[s, r, o, t], [], ...], [], ...]
    valid_list, valid_times = utils.split_by_time(data.valid)
    test_list, test_times = utils.split_by_time(data.test) # 列表，按时间戳划分的事实array：[[[s, r, o, t], [], ...], [], ...]
    ...
    test_model_name = "ly{}-his{}-dp{}|{}|{}|{}-gpu{}"\
        .format(args.n_layers, args.train_history_len,
                args.dropout, args.input_dropout, args.hidden_dropout, args.feat_dropout, args.gpu)
    test_state_file = "../models/{}/{}/{}".format(args.dataset, dir_name, test_model_name) 
    utils.mkdir_if_not_exist(test_state_file)
    ...
    # build hyper-relation graph
    print('-----------build graph and construct hyper-relation graph---------------')
    local_hyperg_path = "../../case/src/"
    node_g = local_hyperg_path + "node_graph/" + args.dataset + ".pkl"
    rel_g = local_hyperg_path + "rel_graph/" + args.dataset + ".pkl"
    if os.path.exists(node_g) and os.path.exists(rel_g):
        with open(node_g, 'rb') as f:
            glist = pickle.load(f)
        with open(rel_g, 'rb') as f:
            super_glist = pickle.load(f)
    else:
        # 文件夹不存在的话创建
        utils.mkdir_if_not_exist(node_g)
        utils.mkdir_if_not_exist(rel_g)
        glist = []
        super_glist = []
        for idx, snap in enumerate(tqdm(train_list + valid_list + test_list)):
            node_sub_g = utils.build_sub_graph(num_nodes, num_rels, snap, use_cuda, args.segnn, args.gpu)
            
            rel_head, rel_tail = utils.get_relhead_reltal(snap, num_nodes, num_rels)
            super_sub_g = utils.build_super_g(num_rels, rel_head, rel_tail, use_cuda, args.segnn, args.gpu)
            
            glist.append(node_sub_g)
            super_glist.append(super_sub_g)
        
        with open(node_g, 'wb') as f:
            pickle.dump(glist, f)
        with open(rel_g, 'wb') as f:
            pickle.dump(super_glist, f)
    print("length of graph list: {}".format(len(glist)))
    ...
    if args.add_static_graph:
        ...
        r_to_e = static_graph.r_to_e
        static_graph = static_graph.to(args.gpu)
        static_graph.r_to_e = torch.from_numpy(np.array(r_to_e))
    ...
    if args.test_valid and os.path.exists(test_state_file):
        ...
        model_name = "ly{}-his{}-dp{}|{}|{}|{}-gpu{}" \
            .format(args.n_layers, args.train_history_len,
                    args.dropout, args.input_dropout, args.hidden_dropout, args.feat_dropout, args.gpu)
        model_state_file = '../models/{}/{}/{}'.format(args.dataset, dir_name, model_name)
        utils.mkdir_if_not_exist(model_state_file)
        ...
        for epoch in range(args.n_epochs): # 对于每一个epoch
            ...
            for train_sample_num in tqdm(idx): # 对于每一个时间戳（idx编号是打乱的，但是还对应相应的第几个时间戳）
                ...
                if train_sample_num - args.train_history_len<0: # 当前时间戳的历史深度不够train_history_len
                    ...
                    left_idx = train_sample_num - args.train_history_len

                history_glist = glist[left_idx : train_sample_num]
                history_super_glist = super_glist[left_idx : train_sample_num]
                graph_to_device(history_glist, history_super_glist, use_cuda, args.gpu)
            ...
            if epoch and epoch % args.evaluate_every == 0:
                ...
                if mrr_raw_ent_rel < best_mrr:
                    if epoch >= args.n_epochs or epoch - best_epoch >= 10:
                        break
                else:
